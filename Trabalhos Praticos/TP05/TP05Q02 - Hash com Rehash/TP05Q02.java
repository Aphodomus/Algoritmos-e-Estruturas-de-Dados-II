...
class ReHash {
    ...
    public ReHash(int tamTabela) {
        this.tamanhoTabela = tamTabela;
        this.tabela = new Musica[tamTabela];
    ...
    public boolean pesquisar(String elemento) {
        ...
        if (elemento != null) {
            ...
            if (tabela[posicao].getId().compareTo(elemento) == 0) {
                ...
                posicao = rehash(pesquisarMusica(elemento));

                comparisions++;
                if (tabela[posicao].getId() != null) {
                    for (int i = 0; i < tamanhoTabela; i++) { //Pode ter um erro, pode ser tamanho -1
                        comparisions++;
                        if (tabela[i].getId().compareTo(elemento) == 0) {
                            resp = true;
                            i = tamanhoTabela;
                        }
                    }
...
public class TP05Q02 {
    ...
    public static ReHash searchIdInReturnBinaryTree(Musica[] musica, ReHash hash, String[] id, int numEntrada) {
        boolean achou = false;
        int w = 0;
