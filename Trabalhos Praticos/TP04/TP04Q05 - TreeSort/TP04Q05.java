...
class TreeSort {
    ...
    // funcao para organizar a arvore
    public Musica[] sort() {
    ...
    public void sort(No no, Musica[] musicas) {
        comparisons++;
        if (no != null) {
            sort(no.esq, musicas);
            musicas[n++] = no.elemento;
            sort(no.dir, musicas);
        }
    }
...
public class TP04Q05 {
    ...
    public static TreeSort searchIdInReturnTreeSort(Musica[] musica, TreeSort arvore, String[] id, int numEntrada) {
        boolean achou = false;
        int w = 0;
    ...
    public static void main(String[] args) throws Exception {
        ...
        TreeSort music = new TreeSort();

        //Iniciar o relogio
        time.start();

        //Criar as musicas pedidas
        music = searchIdInReturnTreeSort(m, music, entrada, i);

        Musica[] resp = music.sort();

        for (int p = 0; p < resp.length; p++) {
            MyIO.println(resp[p].getId() + " " + resp[p].getNome());
        }
