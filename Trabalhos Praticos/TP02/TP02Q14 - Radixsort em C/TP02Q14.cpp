...
void createLog(clock_t inicio, clock_t fim) {
    FILE* arquivo = fopen("matricula_radixsort.txt", "w");
...
Musica maxValue(Musica *music, int n) {
    Musica *max = (Musica*) malloc(1 * sizeof(Musica));
    max[0] = music[0];

    for (int i = 1; i < n; i++) {
        comparisons = comparisons + 1;
        if (music[i].duration_ms > max[0].duration_ms) {
            moves = moves + 1;
            max[0] = music[i];
        }
    }
...
//Counting sort
void countSort(Musica *music, int n, int exp) {
    Musica *output = (Musica*) malloc(n * sizeof(Musica));
    int count[10] = {0};
    int i;

    for (i = 0; i < n; i++) {
        count[(music[i].duration_ms / exp) % 10]++;
    }
    ...
    for (i = n - 1; i >= 0; i--) {
        moves = moves + 1;
        output[count[(music[i].duration_ms / exp) % 10] - 1] = music[i];
        count[(music[i].duration_ms / exp) % 10]--;
    }
...
//Ordenar um array por meio do Radix sort
void sortByRadixSort(Musica *music, int n) {
    Musica *max = (Musica*) malloc(1 * sizeof(Musica));

    //Pegar o maior valor do array
    max[0] = maxValue(music, n);

    //Fazer um counting sort para todo digito
    for (int exp = 1; (max[0].duration_ms / exp ) > 0; exp *= 10) {
        countSort(music, n, exp);
    }
}
